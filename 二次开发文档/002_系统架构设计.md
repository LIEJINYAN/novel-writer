...
| 编辑器组件 | `ui/editor/` | 富文本编辑、Markdown 编辑、大纲视图 | 是 |
...
|---------|------|
| `main()` | 应用入口函数 |
| `NovelWriterApplication` | 应用主类，继承 QApplication |
...
| 类名 | 文件 | 职责 |
|------|------|------|
| `TextEditor` | `text_editor.py` | 富文本编辑器，基于 QTextEdit |
| `MarkdownEditor` | `markdown_editor.py` | Markdown 编辑器，双栏预览 |
| `OutlineView` | `outline_view.py` | 大纲视图，章节树状结构 |
| `EditorTabWidget` | `editor_tabs.py` | 多标签页编辑器 |
...
class WritingAIService:
    async def continue_writing(self, context: WritingContext) -> str: ...
    async def polish_text(self, text: str, style: str) -> str: ...
    async def generate_dialogue(self, characters: List[str], scene: str) -> str: ...
    async def generate_outline(self, idea: str, method: str) -> Outline: ...
```
...
| 状态类型 | 存储位置 | 说明 |
|----------|----------|------|
| **应用配置** | `config.json` | 全局设置，如主题、语言、AI 配置 |
