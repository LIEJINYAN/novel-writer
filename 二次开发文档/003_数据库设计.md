...
| `content` | TEXT (MEDIUMTEXT*) | - | 章节正文（HTML/Markdown） | NULL |
...
- 数据库设计文档中标注了 `MEDIUMTEXT`，但 **SQLite 原生不支持 `MEDIUMTEXT` 类型**，实际会映射为 `TEXT`。该标注仅作为语义提示，表示该字段预期存储较长的文本内容。如需在 MySQL 等支持 `MEDIUMTEXT` 的数据库中部署，可进行相应转换。
