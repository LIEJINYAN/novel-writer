"""关系/派系追踪器 - 管理角色关系、派系和关系图。

提供核心数据库逻辑，供 services/relationship_service.py 委托调用。
"""

from __future__ import annotations
from typing import Optional
from models import Character, Relationship, RelationshipChange, Faction, FactionMember
from models.database import DatabaseManager


class RelationshipTracker:
    """关系/派系追踪器 - 管理关系、变化历史和派系。"""
    ...
    def add_relationship(self, project_id: int, character_a_id: int, character_b_id: int,
                         **data) -> Relationship:
        """添加关系，自动记录变化历史。"""
    ...
    def update_relationship(self, relationship_id: int, **data) -> Optional[Relationship]:
        """更新关系，自动记录变化。"""
        session = self._db.get_project_session()
        try:
    ...
    def list_relationships(self, project_id: int, search: str = "") -> list[dict]:
        """列出关系，支持按角色名搜索。"""
        session = self._db.get_project_session()
        try:
    ...
    def add_faction(self, project_id: int, name: str, **data) -> Faction:
        """添加派系。"""
        session = self._db.get_project_session()
        try:
    ...
    def add_faction_member(self, faction_id: int, character_id: int,
    ...
    def remove_faction_member(self, faction_id: int, character_id: int) -> bool:
        """移除派系成员。"""
        session = self._db.get_project_session()
        try:
    ...
    def get_relationship_graph(self, project_id: int) -> dict:
        """获取关系图数据。

        Returns:
            dict: 包含 nodes（角色节点）和 edges（关系边）的图结构。
        """
        session = self._db.get_project_session()
        try:
